...
def vote(vote: schemas.Vote, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if vote.dir == 1:
        ...
        new_vote = models.Vote(post_id = int(vote.post_id), user_id = int(current_user.id))
        db.add(new_vote)
        db.commit()
        return {"message": "Successfully added vote"}
    else:
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"User {current_user.id} has not yet voted for {vote.post_id}.")
